...
bool DataTemplate::begin() {
	...
	if (!checkConfig(config_path)) {
		...
		// Load settings
		result = setConfig(loadConfig(config_path), false);
	}
	return result;
}
...
JsonDocument DataTemplate::getConfig() {
	// Allocate the JSON document
	JsonDocument doc;
...
/// @param config A JSON document of the configuration settings
/// @param save If the configuration should be saved to a file
/// @return True on success
bool DataTemplate::setConfig(JsonDocument config, bool save) {
	// Assign loaded values
	current_config.template_start = config["template_start"].as<String>();
	current_config.template_end = config["template_end"].as<String>();
	current_config.template_data = config["template_data"].as<String>();
	if (save) {
